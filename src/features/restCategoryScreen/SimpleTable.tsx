...
import {ItemCatGetRequest,ItemCatDeleteRequest} from "../../redux/actions/itemCategoryAction";
...
let DataTableSort: React.FunctionComponent<Props> = props => {
    ...
    const nameTemplate = (rowData) => {
        if (rowData["category_name"]!==undefined && rowData["category_name"] !== null){
            return <>
            <span className="customFont2 cData">{rowData['category_name']}</span>
            </> ;
        }
        return <>
        ...
        <div className="datatable-responsive">
            <div className="card tableCard" >
                <DataTable ref={dt} value={props.itemCats.data?props.itemCats.data.data:[]} className="p-datatable-responsive" globalFilter={globalFilter} header={header} paginator rows={7}>
                    <Column field="category_name" header="Name" body={nameTemplate} sortable></Column>
            ...
            <Dialog header="Delete category" visible={showDelModal} className="dialogSize" modal footer={deleteCatDialogFooter} onHide={()=>{setShowDelModal(false)}}>
                ...
                <div className="dialogCss">
                <span>
                                        Are you sure you want to delete <b>{currentA && currentA['category_name']}</b> category?
                                    </span>
                </div>
                </Row>
            </Dialog> 
...
const mapDispatchToProps = dispatch => ({
    ...
    deletecat:(d)=>{dispatch(ItemCatDeleteRequest(d))}
